  ...
  const generatePDF = () => {
    ...
    const pdfContent = `
      ...
      <html>
        ...
        <body>
          <div class="header">
            <h1 style="color: #1f2937; margin-bottom: 10px;">${selectedTrip.name}</h1>
            <h2 style="color: #3b82f6; margin-bottom: 20px;">Trip Itinerary</h2>
            ${selectedTrip.coverPhoto ? `<img src="${selectedTrip.coverPhoto}" alt="${selectedTrip.name}" class="trip-image">` : ''}
            <p style="color: #6b7280; margin-top: 15px;">Generated on ${format(new Date(), 'MMMM dd, yyyy')}</p>
          </div>
              ...
              stops.map((stop, index) => `
                <div class="stop-item">
                  <div class="stop-header">
                    ...
                    <div class="stop-title">${stop.cityName}, ${stop.country}</div>
                  </div>
                  <div class="stop-details">
                    <div><strong>Arrival:</strong> ${format(new Date(stop.arrivalDate), 'MMM dd, yyyy')}</div>
                    <div><strong>Departure:</strong> ${format(new Date(stop.departureDate), 'MMM dd, yyyy')}</div>
                    ${stop.notes ? `<div><strong>Notes:</strong> ${stop.notes}</div>` : ''}
                  </div>
                </div>
              `).join('') : 
              '<div class="no-stops">No stops planned yet</div>'
            }
          </div>

          <div class="footer">
            <p>Generated by GlobeTrotter - Plan. Explore. Experience.</p>
            <p>${selectedTrip.name} • ${format(new Date(selectedTrip.startDate), 'MMM dd')} - ${format(new Date(selectedTrip.endDate), 'MMM dd, yyyy')}</p>
          </div>
