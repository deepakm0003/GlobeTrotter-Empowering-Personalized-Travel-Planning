...
import { fetchMyTrips, createCalendarEvent, updateCalendarEvent, deleteCalendarEvent } from '../../data/mockData';
...
const TripCalendar: React.FC = () => {
  ...
  const handleEditActivity = (activityId: number) => {
    setEditingActivity(activityId.toString());
  };

  const handleSaveActivity = async (activityId: number, updatedActivity: Partial<Activity>) => {
    try {
      await updateCalendarEvent(activityId.toString(), {
        name: updatedActivity.name || '',
        description: updatedActivity.description || '',
        duration: updatedActivity.duration || 2,
        cost: updatedActivity.cost || 0,
        category: updatedActivity.category || 'sightseeing'
      }, user?.id);
  ...
  const handleDeleteActivity = async (activityId: number) => {
    try {
      await deleteCalendarEvent(activityId.toString(), user?.id);
  ...
  const handleAddEvent = async () => {
    ...
    try {
      ...
      const newActivity = await createCalendarEvent({
      ...
      console.log('Event added successfully:', newActivity); // Debug log
                    ...
                    {trip.stops?.map((stop, index) => (
                      <div key={stop.id} className="ml-4 mb-2">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm text-white">{stop.cityName}</span>
                        </div>
          ...
          <div className="bg-slate-800 rounded-xl p-6 w-full max-w-md">
            ...
            <div className="flex space-x-3 mt-6">
              ...
              <button
                onClick={() => {
                  if (editingActivity) {
                    handleSaveActivity(parseInt(editingActivity), {});
                    setEditingActivity(null);
                  }
                }}
                className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center space-x-2"
